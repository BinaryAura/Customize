...
public abstract class HudItemTexturedBar extends HudItemBar {

	public HudItemTexturedBar(String name) {
		super(name);
		
	}
	
	//	TODO: Revise to be able to grow the bar?
	@Override
	public void renderHUDItem(int x, int y) {
		...
		for(int i = 0; i < layers.getAmount() + 1; i++) {
			int fill = MathHelper.ceiling_float_int(getAmount(i)*getTotal());
	...
	@Override
	protected int getColor(int layer) {
		return 0;
	}
	
	@Override
	protected int getAlpha(int layer) {
		return 0;
	}
